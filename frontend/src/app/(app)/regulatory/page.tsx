...
export default function RegulatoryPage() {
  ...
  const dataRequests = analysis?.result.case_specific_analysis?.data_requests.filter((item) => /regulatory|ABS|science|quality/i.test(item.blocks)) ?? [];
  return (
    <div className="py-8 md:py-10">
      ...
      {!regulatory ? <AnalysisState loading={loading} error={error} /> : (
        <>
          ...
          {dataRequests.length > 0 && <section className="mt-8 rounded-3xl border border-warm/30 bg-warm-subtle p-6"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-warm">Fact-specific blockers</p><h2 className="mt-2 text-2xl font-semibold">Resolve these before a pathway conclusion</h2><div className="mt-5 grid gap-3 md:grid-cols-2">{dataRequests.map((item) => <div key={item.field} className="rounded-xl border border-warm/20 bg-surface/70 p-4"><p className="text-sm font-semibold">{item.field.replaceAll("_", " ")}</p><p className="mt-2 text-xs leading-5 text-ink-muted">{item.question}</p><p className="mt-2 font-mono text-[9px] uppercase text-warm">Blocks: {item.blocks}</p></div>)}</div></section>}
