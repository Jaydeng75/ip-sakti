    ...
    <div className="py-8 md:py-10">
      ...
      {!result ? <AnalysisState loading={loading} error={error} /> : (
        <>
          ...
          <section className="mt-8 rounded-3xl border border-border bg-surface p-6 md:p-8"><div className="flex flex-col justify-between gap-3 md:flex-row md:items-end"><div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-accent">How to strengthen this innovation</p><h2 className="mt-2 text-2xl font-semibold">Highest-impact technical actions</h2></div><Link href="/design-around" className="text-sm font-semibold text-accent">Open full technical advisor →</Link></div><div className="mt-6 grid gap-3 lg:grid-cols-3">{result.case_specific_analysis.technical_advisory.strength_actions.slice(0, 3).map((action) => <article key={action.rank} className="rounded-2xl border border-border bg-background p-5"><div className="flex items-center justify-between gap-3"><span className="font-mono text-[10px] text-accent">ACTION {String(action.rank).padStart(2, "0")}</span><span className="rounded-full bg-warm-subtle px-2.5 py-1 font-mono text-[9px] uppercase text-warm">{action.impact}</span></div><h3 className="mt-4 text-base font-semibold leading-6">{action.title}</h3><p className="mt-3 text-xs leading-5 text-ink-muted">{action.why}</p><p className="mt-4 font-mono text-[9px] uppercase text-accent">Output</p><p className="mt-2 text-xs leading-5">{action.deliverable}</p></article>)}</div></section>
...
function SignalDetail({ card }: { card: RiskCard }) { return <div className="mt-5 rounded-2xl border border-accent/30 bg-background p-5"><div className="rounded-xl border border-accent/20 bg-accent-subtle p-4"><p className="font-mono text-[9px] uppercase tracking-[0.15em] text-accent">1 / Finding</p><p className="mt-2 text-sm font-semibold leading-6">{card.finding ?? card.primary_finding}</p></div><div className="mt-5 grid gap-5 md:grid-cols-3"><TextDetail title="2 / Why" text={card.why ?? card.summary} /><DetailColumn title="3 / Evidence" items={card.evidence_basis ?? []} /><TextDetail title="4 / Fix" text={card.fix ?? "Qualified human review is required."} /></div><details className="mt-5 border-t border-border pt-4"><summary className="cursor-pointer text-xs font-semibold text-accent">Additional screening signals</summary><div className="mt-4 grid gap-5 md:grid-cols-2 xl:grid-cols-4"><DetailColumn title="Positive signals" items={card.positive_signals ?? []} /><DetailColumn title="Negative signals" items={card.negative_signals ?? []} /><DetailColumn title="Missing evidence" items={card.missing_evidence ?? []} /><DetailColumn title="What changes it" items={card.what_changes_score ?? []} /></div></details></div>; }
...
function TextDetail({ title, text }: { title: string; text: string }) { return <div><p className="font-mono text-[9px] uppercase tracking-[0.15em] text-accent">{title}</p><p className="mt-3 text-xs leading-5 text-ink-muted">{text}</p></div>; }
