...
export default function AskPage() {
  ...
  async function ask(prompt = question) {
    ...
    setError(null);
    try {
      const response = await caseApi.ask(currentCase.backendId, trimmed, language);
    ...
    <div className="py-8 md:py-10">
      ...
      {!currentCase.backendId ? (
        ...
        <>
          ...
          <div className="mt-4 flex flex-wrap gap-2">{quickQuestions.map((prompt) => <button key={prompt} type="button" onClick={() => void ask(prompt)} className="rounded-full border border-border bg-surface px-3 py-2 text-xs text-ink-muted transition hover:border-accent hover:text-accent">{prompt}</button>)}</div>
          ...
          <section className="mt-6 grid min-h-[680px] gap-6 lg:grid-cols-[1fr_410px]">
            <div className="flex min-h-[680px] flex-col overflow-hidden rounded-3xl border border-border bg-surface">
              ...
              <div className="flex-1 space-y-5 overflow-y-auto p-6">
                {messages.map((message, index) => (
                  <button key={`${message.role}-${index}`} type="button" onClick={() => { if (message.response) { setActiveResponse(message.response); setSelectedSource(message.response.citations[0] ?? null); } }} className={`block text-left ${message.role === "user" ? "ml-auto max-w-[82%] rounded-2xl bg-accent px-5 py-4 text-white" : "max-w-[90%] rounded-2xl bg-background px-5 py-4 text-ink"}`}>
                    ...
                    {message.response && <span className="mt-3 flex flex-wrap gap-2"><Badge>{message.response.claim_type.replaceAll("_", " ")}</Badge><Badge>{Math.round(message.response.confidence * 100)}% confidence</Badge><Badge>{message.response.citations.length} sources</Badge></span>}
                  </button>
                ))}
              ...
              <div className="border-t border-border p-4"><div className="rounded-2xl border border-border bg-background p-3 focus-within:border-accent"><textarea value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void ask(); } }} placeholder="Ask a legal, evidence, patent, ABS or market question…" className="min-h-24 w-full resize-none bg-transparent px-2 py-2 text-sm leading-6 outline-none" /><div className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-2 pt-3"><label className="flex items-center gap-2 text-[11px] text-ink-muted">Response language<select value={language} onChange={(event) => setLanguage(event.target.value)} className="rounded-lg border border-border bg-surface px-2 py-1 text-xs text-ink"><option>English</option><option>Hindi</option><option>Tamil</option><option>Bengali</option><option>Marathi</option></select></label><button type="button" onClick={() => void ask()} disabled={!question.trim() || asking} className="rounded-xl bg-accent px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-40">Ask →</button></div></div></div>
            </div>
            ...
            <aside className="flex min-h-[680px] flex-col overflow-hidden rounded-3xl border border-border bg-surface">
              ...
              {activeResponse && <div className="border-t border-border p-5"><p className="text-xs leading-5 text-ink-muted">{activeResponse.limitations[0]}</p></div>}
            </aside>
          </section>
        </>
      )}
