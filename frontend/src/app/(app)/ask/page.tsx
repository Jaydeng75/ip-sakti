...
import { AskResponse, Citation, caseApi, openCitation } from "@/lib/api";
...
export default function AskPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", text: "Ask about this innovation case. I will route legal, patent, traditional-knowledge, ABS and scientific questions to their own evidence pipelines and show what supports each answer." },
  ]);
          ...
          <section className="mt-6 grid min-h-[680px] gap-6 lg:grid-cols-[1fr_410px]">
              ...
              <div className="flex-1 space-y-5 overflow-y-auto p-6">
                {messages.map((message, index) => (
                  <button key={`${message.role}-${index}`} type="button" onClick={() => { if (message.response) { setActiveResponse(message.response); setSelectedSource(message.response.citations[0] ?? null); } }} className={`block text-left ${message.role === "user" ? "ml-auto max-w-[82%] rounded-2xl bg-accent px-5 py-4 text-white" : "max-w-[90%] rounded-2xl bg-background px-5 py-4 text-ink"}`}>
                    ...
                    <span className="mt-2 block whitespace-pre-line text-sm leading-7">{message.text}</span>
                    {message.response && <span className="mt-3 flex flex-wrap gap-2"><Badge>{message.response.claim_type.replaceAll("_", " ")}</Badge><Badge>{message.response.intent.replaceAll("_", " ")}</Badge><Badge>Evidence confidence · {message.response.confidence_label}</Badge><Badge>{message.response.citations.length} sources</Badge><Badge>{message.response.reasoning.status === "grounded_llm" ? "Grounded LLM reasoning" : "Deterministic guardrails"}</Badge>{message.response.output_translation.machine_translated && <Badge>IndicTrans2 translation</Badge>}</span>}
                  </button>
                ))}
                {asking && <div className="max-w-[90%] animate-pulse rounded-2xl bg-background px-5 py-4 text-sm text-ink-muted">Classifying the question and matching case-level evidence…</div>}
            ...
            <aside className="flex min-h-[680px] flex-col overflow-hidden rounded-3xl border border-border bg-surface">
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {!activeResponse ? <p className="rounded-2xl border border-dashed border-border p-5 text-sm leading-6 text-ink-muted">Ask a question to retrieve the relevant evidence type without mixing legal rules with efficacy evidence.</p> : <>{activeResponse.evidence_summary && <div className="mb-4 grid grid-cols-2 gap-2">{Object.entries(activeResponse.evidence_summary).map(([label, value]) => <div key={label} className="rounded-xl border border-border bg-background p-3"><p className="text-xl font-semibold text-accent">{value}</p><p className="mt-1 text-[9px] uppercase leading-4 text-ink-muted">{label.replaceAll("_", " ")}</p></div>)}</div>}{activeResponse.citations.length ? <div className="space-y-2">{activeResponse.citations.map((citation) => <button key={citation.id} type="button" onClick={() => setSelectedSource(citation)} className={`w-full rounded-2xl border p-4 text-left transition ${selectedSource?.id === citation.id ? "border-accent bg-accent-subtle" : "border-border hover:bg-background"}`}><div className="flex items-start justify-between gap-3"><p className="text-sm font-semibold leading-5">{citation.title}</p><span className="font-mono text-[9px] text-ink-muted">{citation.jurisdiction}</span></div><p className="mt-2 text-xs text-ink-muted">{citation.authority}</p><div className="mt-3 flex flex-wrap gap-2"><Badge>{citation.support_status.replaceAll("_", " ")}</Badge><Badge>{citation.effective_date}</Badge></div></button>)}</div> : <p className="rounded-2xl border border-warm/30 bg-warm-subtle p-5 text-sm leading-6 text-warm">Safe abstention: no relevant source supported this answer.</p>}</>}
              </div>
              {selectedSource && <div className="border-t border-border bg-background p-5"><p className="font-mono text-[9px] uppercase tracking-[0.16em] text-ink-muted">Selected source</p><p className="mt-2 text-sm font-semibold">{selectedSource.title}</p>{selectedSource.locator && <p className="mt-1 font-mono text-[9px] uppercase text-accent">{selectedSource.locator}</p>}<p className="mt-2 text-xs leading-5 text-ink-muted">{selectedSource.excerpt}</p><button type="button" onClick={() => void openCitation(selectedSource)} className="mt-4 inline-flex text-xs font-semibold text-accent hover:underline">Open {selectedSource.source_type === "scientific_literature" ? "scientific source" : selectedSource.source_type === "case_document" ? "case evidence" : "official source"} ↗</button></div>}
              {activeResponse && <div className="border-t border-border p-5"><p className="text-xs font-semibold">Evidence confidence: {activeResponse.confidence_label}</p><p className="mt-1 text-xs leading-5 text-ink-muted">{activeResponse.confidence_basis}</p><div className="mt-3 flex flex-wrap gap-2"><Badge>{activeResponse.reasoning.status === "grounded_llm" ? "Grounded LLM reasoning" : activeResponse.reasoning.fallback_used ? "Deterministic fallback" : "Deterministic guardrails"}</Badge>{activeResponse.reasoning.model && <Badge>{activeResponse.reasoning.model.split("/").at(-1)}</Badge>}</div>{activeResponse.reasoning.status === "grounded_llm" && <details className="mt-4 rounded-xl border border-accent/20 bg-accent-subtle p-3"><summary className="cursor-pointer text-xs font-semibold text-accent">Validated reasoning details</summary><div className="mt-3 space-y-3 text-xs leading-5 text-ink-muted">{activeResponse.reasoning.finding && <p>{activeResponse.reasoning.finding}</p>}{activeResponse.reasoning.weak_points.length > 0 && <div><p className="font-semibold text-ink">Weak points</p><ul>{activeResponse.reasoning.weak_points.map((item) => <li key={item}>• {item}</li>)}</ul></div>}{activeResponse.reasoning.missing_evidence.length > 0 && <div><p className="font-semibold text-ink">Missing evidence</p><ul>{activeResponse.reasoning.missing_evidence.map((item) => <li key={item}>• {item}</li>)}</ul></div>}{activeResponse.reasoning.recommended_actions.length > 0 && <div><p className="font-semibold text-ink">Recommended actions</p><ul>{activeResponse.reasoning.recommended_actions.map((item) => <li key={item}>• {item}</li>)}</ul></div>}</div></details>}<details className="mt-4 rounded-xl border border-border bg-surface p-3"><summary className="cursor-pointer text-xs font-semibold text-accent">Methodology</summary><ul className="mt-3 space-y-2 text-xs leading-5 text-ink-muted">{activeResponse.methodology.map((item) => <li key={item}>• {item}</li>)}</ul></details><div className="mt-4 flex flex-wrap gap-2"><Badge>{activeResponse.output_translation.machine_translated ? "Machine translated · IndicTrans2" : `Output · ${activeResponse.output_translation.status}`}</Badge><Badge>{activeResponse.response_language}</Badge></div>{activeResponse.output_translation.machine_translated && activeResponse.authoritative_answer !== activeResponse.answer && <details className="mt-4 rounded-xl border border-border bg-surface p-3"><summary className="cursor-pointer text-xs font-semibold text-accent">View authoritative English answer</summary><p className="mt-3 text-xs leading-5 text-ink-muted">{activeResponse.authoritative_answer}</p></details>}<p className="mt-4 text-xs leading-5 text-ink-muted">{activeResponse.limitations[0]}</p></div>}
            </aside>
          </section>
        </>
      )}
