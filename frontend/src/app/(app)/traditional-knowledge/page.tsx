...
const TKDL_GLOBAL_SEARCH_URL = "https://www.tkdl.res.in/tkdl/langdefault/common/Global_Search.asp?GL=Eng";
      ...
      {!graph ? <AnalysisState loading={loading} error={error} /> : (
        <>
          {tk && <section className="mt-8 rounded-3xl border border-border bg-surface p-6 md:p-8">
            <div className="flex flex-col justify-between gap-5 md:flex-row md:items-start"><div><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-accent">Exact passage register</p><h2 className="mt-2 text-2xl font-semibold">Formulations and locations, not generic matches</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-ink-muted">{tk.limitation}</p></div><a href={TKDL_GLOBAL_SEARCH_URL} target="_blank" rel="noreferrer" className="rounded-xl border border-accent px-4 py-2 text-xs font-semibold text-accent">Open TKDL Global Search ↗</a></div>
            <div className="mt-5 rounded-xl border border-border bg-background p-4"><p className="font-mono text-[9px] uppercase tracking-[0.14em] text-accent">Suggested search terms</p><p className="mt-2 text-xs leading-5 text-ink-muted">{tk.query || "Add botanical names, formulation and intended use to generate a case-specific query."}</p></div>
