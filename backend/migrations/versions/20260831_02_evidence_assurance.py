...
def upgrade() -> None:
    ...
    evidence_columns = {column["name"] for column in inspector.get_columns("evidence_chunks")}
    missing_evidence_columns = [
        column
        for column in [
            sa.Column("embedding_vector", vector_type, nullable=True),
            sa.Column("embedding_provider", sa.String(length=60), server_default="deterministic", nullable=False),
            sa.Column("embedding_model", sa.String(length=180), server_default="blake2b-feature-hash", nullable=False),
            sa.Column("embedding_revision", sa.String(length=100), server_default="v1", nullable=False),
        ]
        if column.name not in evidence_columns
    ]
    if missing_evidence_columns:
        with op.batch_alter_table("evidence_chunks") as batch:
            for column in missing_evidence_columns:
                batch.add_column(column)
    if postgres:
        op.execute(
            "CREATE INDEX IF NOT EXISTS ix_evidence_chunks_embedding_hnsw ON evidence_chunks "
            "USING hnsw (embedding_vector vector_cosine_ops) WHERE embedding_vector IS NOT NULL"
        )

    existing_tables = set(inspector.get_table_names())
    if "reindex_jobs" not in existing_tables:
        op.create_table(
            "reindex_jobs",
    ...
    if "source_snapshots" not in existing_tables:
        op.create_table(
            "source_snapshots",
            sa.Column("id", sa.Integer(), primary_key=True),
